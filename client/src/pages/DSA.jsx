...
import { FaCode, FaCheckCircle, FaSearch } from 'react-icons/fa';
...
const DSA = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const { data } = await api.get('/posts/type/DSA');
                setPosts(data);
            } catch (error) {
                console.error(error);
            } finally {
        ...
        <div className="min-h-screen bg-gray-50 pt-24 pb-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
                <div className="text-center mb-12">
                    <div className="inline-block p-3 rounded-2xl bg-indigo-100 text-indigo-600 mb-4 shadow-inner">
                        <FaCode size={32} />
                    </div>
                    <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4 font-outfit">DSA Problems</h1>
                    <p className="text-xl text-gray-600 max-w-2xl mx-auto">Curated list of Data Structures and Algorithms problems to crack your technical interviews.</p>
                </div>
                        ...
                        {filteredPosts.length > 0 ? (
                            <ul className="divide-y divide-gray-100">
                                {filteredPosts.map((post, idx) => (
                                    <li key={post._id} className="group hover:bg-indigo-50/30 transition duration-200">
                                        <Link to={`/post/${post.slug}`} className="block p-6 sm:p-8">
                                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-3 mb-2">
                                                        <div className="text-gray-400 font-mono text-sm">#{String(idx + 1).padStart(3, '0')}</div>
                                                        <h3 className="text-xl font-bold text-gray-900 group-hover:text-indigo-600 transition-colors font-outfit">{post.title}</h3>
                                                    </div>
                                                ...
                                                <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                                                    ...
                                                    <FaCheckCircle className="text-gray-300 group-hover:text-indigo-400 transition-colors text-xl" />
                                                </div>
                                            </div>
                                        </Link>
                                    </li>
